...
@app.route('/picture_cat/<id>')
def picture_cat(id):
    per_page = 6
    page = request.args.get('page', 1, type=int)
    pictures = AddPicture.query.filter_by(category_id = id)\
                                  .order_by(AddPicture.created_at.desc())\
                                  .paginate(page=page, per_page=per_page, error_out=False)
    return render_template("home/picture_cat.html", title='picture', pictures=pictures, id=id)
